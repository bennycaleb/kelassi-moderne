...
function Directors() {
  ...
  const canManage = ['admin', 'superadmin'].includes(current.role);

  async function refresh() {
    const data = await api('/api/users');
    setDirectors((data.users || []).filter((user) => user.role === 'director'));
  }
  ...
  async function submit(event) {
    ...
    setError('');
    try {
      const result = await api('/api/users', { method: 'POST', body: { ...form, role: 'director' } });
    ...
    <div>
      ...
      {credentials && (
        <div className="credentials-box">
          ...
          <p>Il se connecte sur la même page que vous, puis arrive dans <b>Direction de l’établissement</b>.</p>
        </div>
      )}
      ...
      <div className="panel">
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {directors.length === 0 ? (
                <tr><td colSpan="3">Aucun compte D.E. pour le moment. Cliquez sur « Ajouter un D.E. ».</td></tr>
              ) : directors.map((user) => (
                <tr key={user.id}>
      ...
      {open && (
        <Modal title="Ajouter un D.E." onClose={() => setOpen(false)}>
